...
export interface ChatAction {
  type: 'addPrompt' | 'removePrompt' | 'clearAll' | 'setBpm' | 'setGlobal';
  data?: any;
}
...
export class ChatHelper {
  ...
  constructor(ai: GoogleGenAI, model: string = 'gemini-3-flash-preview') {
    this.ai = ai;
    this.model = model;
  }

  async sendCommand(text: string, currentPrompts: Prompt[], currentBpm: number | 'Auto'): Promise<ChatAction[]> {
      ...
      if (calls) {
        for (const call of calls) {
          if (call.name === 'add_prompt') {
            ...
            // Allow dynamic weights, but nudge extremely low ones up if likely a mistake
            if (w < 0.2) w = 0.5; 
